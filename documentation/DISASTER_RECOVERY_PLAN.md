...
This document outlines the disaster recovery (DR) procedures for D3VONN's production environment.
...
aws rds describe-db-instances --db-instance-identifier d3vonn-postgres-production \
...
aws rds promote-read-replica \
  --db-instance-identifier d3vonn-postgres-replica-production
```

2. **Update Database Connection**:
```bash
kubectl set env deployment/d3vonn-api \
  DATABASE_URL=postgresql://admin:password@[new-endpoint]:5432/d3vonndb
```
...
```bash
aws eks describe-cluster --name d3vonn-eks-production
kubectl get nodes
...
```bash
aws rds describe-db-instances --db-instance-identifier d3vonn-postgres-dr-production
```
...
4. **Scale Up Capacity**:
```bash
kubectl scale deployment d3vonn-api --replicas=3
kubectl scale deployment d3vonn-worker --replicas=5
```
...
```bash
psql -h [endpoint] -U admin -d d3vonndb -c "SELECT COUNT(*) FROM users;"
```
...
kubectl set env deployment/d3vonn-api \
  DATABASE_URL=postgresql://admin:password@[recovered-endpoint]:5432/d3vonndb
```

#### S3 Data Recovery
1. **Verify Backup Integrity**:
```bash
aws s3 ls s3://d3vonn-backup-production/
```

2. **Restore Data**:
```bash
aws s3 sync s3://d3vonn-backup-production/[backup-date]/ s3://d3vonn-data-production/
```
...
### Contact Information
- **On-call Rotation**: https://d3vonn.pagerduty.com/on-call
