...
This guide covers deployment procedures for the D3VONN platform across different environments and cloud providers.
...
```bash
# Frontend
docker build -f Dockerfile.frontend -t d3vonn-frontend:latest .

# Backend
docker build -f Dockerfile -t d3vonn-backend:latest .
```
...
aws ecr create-repository --repository-name d3vonn-backend
...
# Tag and push image
docker tag d3vonn-backend:latest \
  <account-id>.dkr.ecr.us-east-1.amazonaws.com/d3vonn-backend:latest
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/d3vonn-backend:latest

# Create ECS task definition and service
aws ecs create-service \
  --cluster d3vonn-cluster \
  --service-name backend \
  --task-definition d3vonn-backend \
  --desired-count 2
```
...
az group create --name d3vonn-rg --location eastus

# Create container registry
az acr create --resource-group d3vonn-rg \
  --name d3vonnregistry --sku Basic

# Login to ACR
az acr login --name d3vonnregistry

# Tag and push image
docker tag d3vonn-backend:latest \
  d3vonnregistry.azurecr.io/d3vonn-backend:latest
docker push d3vonnregistry.azurecr.io/d3vonn-backend:latest

# Create container app
az containerapp create \
  --name d3vonn-backend \
  --resource-group d3vonn-rg \
  --image d3vonnregistry.azurecr.io/d3vonn-backend:latest \
  --target-port 8000 \
  --ingress external
```
...
gcloud builds submit --tag gcr.io/your-project-id/d3vonn-backend

# Deploy to Cloud Run
gcloud run deploy d3vonn-backend \
  --image gcr.io/your-project-id/d3vonn-backend \
  --platform managed \
  --region us-central1 \
...
docker pull d3vonn-backend:v1.0.0

# Stop current container
docker stop d3vonn-backend
docker rm d3vonn-backend

# Start previous version
docker run -d --name d3vonn-backend \
  -p 8000:8000 \
  d3vonn-backend:v1.0.0
```
...
**GCP Cloud Run:**
```bash
gcloud run services update-traffic d3vonn-backend \
...
**Docker:**
```bash
docker logs -f d3vonn-backend
docker logs -f d3vonn-frontend
```

**AWS CloudWatch:**
```bash
aws logs tail /ecs/d3vonn-backend --follow
```
