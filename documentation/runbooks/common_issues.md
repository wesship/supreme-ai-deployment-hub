...
This runbook provides step-by-step instructions for resolving common issues in the D3VONN infrastructure.
...
1. **If RDS instance is not "available"**
   ...
   - If needed, restore from latest snapshot:
     ...
     aws rds restore-db-instance-from-db-snapshot \
       --db-instance-identifier d3vonn-postgres-new \
...
3. **If credentials are incorrect**
   ...
   - Reset password if needed:
     ...
     aws secretsmanager update-secret \
       --secret-id d3vonn/db/credentials \
...
1. **Identify affected pods**
   ```bash
   kubectl get pods -n d3vonn -o wide | grep CrashLoopBackOff
   ```

2. **Check pod logs**
   ```bash
   kubectl logs $POD_NAME -n d3vonn
   kubectl logs $POD_NAME -n d3vonn --previous
   ```

3. **Check pod events**
   ```bash
   kubectl describe pod $POD_NAME -n d3vonn
   ```
...
2. **If container can't start due to configuration**
   ```bash
   # Check ConfigMaps
   kubectl get configmap -n d3vonn
   kubectl describe configmap $CONFIGMAP_NAME -n d3vonn
   
   # Fix ConfigMap
   kubectl edit configmap $CONFIGMAP_NAME -n d3vonn
   
   # Restart affected pods
   kubectl rollout restart deployment $DEPLOYMENT_NAME -n d3vonn
   ```

3. **If image pull errors**
   ```bash
   # Check image repository access
   kubectl describe pod $POD_NAME -n d3vonn | grep "Failed to pull image"
   ...
   kubectl set image deployment/$DEPLOYMENT_NAME $CONTAINER_NAME=$CORRECT_IMAGE -n d3vonn
   ```
...
1. **Monitor latency metrics in CloudWatch**
   ...
   aws cloudwatch get-metric-statistics \
     ...
     --metric-name Latency \
     --dimensions Name=ApiName,Value=d3vonn-api \
...
2. **If application pods need scaling**
   ```bash
   # Scale up deployment
   kubectl scale deployment $DEPLOYMENT_NAME --replicas=5 -n d3vonn
   
   # Verify HPA (Horizontal Pod Autoscaler) settings
   kubectl get hpa -n d3vonn
   
   # Update HPA if needed
   kubectl edit hpa $HPA_NAME -n d3vonn
   ```

3. **If network bottlenecks**
   ```bash
   # Check network policies
   kubectl get networkpolicies -n d3vonn
   
   # Check service mesh configuration
   aws appmesh describe-virtual-service \
     --mesh-name d3vonn-mesh-production \
     --virtual-service-name api.d3vonn.local
...
1. **Check App Mesh resources**
   ...
   aws appmesh list-virtual-nodes --mesh-name d3vonn-mesh-production
   aws appmesh list-routes --mesh-name d3vonn-mesh-production
   ```

2. **Check Envoy sidecar logs**
   ```bash
   # Get pod name
   POD=$(kubectl get pods -n d3vonn -l app=api -o jsonpath='{.items[0].metadata.name}')
   
   # Check Envoy logs (assuming Envoy container is named 'envoy')
   kubectl logs $POD -c envoy -n d3vonn
   ```

3. **Check Envoy configuration**
   ```bash
   kubectl exec -it $POD -c envoy -n d3vonn -- curl localhost:9901/config_dump
   ```
...
1. **If resource already exists error**
   ...
   terraform import aws_db_instance.postgres_read_replica d3vonn-postgres-replica-production
   ```
...
2. **Check canary logs**
   ```bash
   kubectl logs -l version=canary -n d3vonn
   ```

3. **Verify canary environment configuration**
   ```bash
   kubectl get configmap -l version=canary -n d3vonn
   ```
...
2. **If configuration issues**
   ...
   kubectl edit configmap canary-config -n d3vonn
   
   # Restart canary pods
   kubectl rollout restart deployment -l version=canary -n d3vonn
   ```
...
2. **Check specific resource configuration**
   ...
   aws s3api get-bucket-encryption --bucket d3vonn-data-production
   aws s3api get-public-access-block --bucket d3vonn-data-production
   ```
...
1. **If encryption is missing**
   ...
   aws rds restore-db-snapshot \
     --db-instance-identifier d3vonn-postgres-encrypted \
     --db-snapshot-identifier pre-encryption-$(date +%Y-%m-%d) \
     --storage-encrypted \
