# D3VONN Production Runbook

**Version:** 2.0.0
**Last Updated:** 2026-05-16

This runbook provides step-by-step procedures for operating, monitoring, and recovering the D3VONN platform in production.
...
# Backend (Kubernetes)
kubectl set image deployment/d3vonn-backend backend=<ECR_URI>:<TAG> -n d3vonn
...
# Example:
bash scripts/rollback.sh d3vonn-backend d3vonn
```
...
```bash
# Backend logs
kubectl logs -f deployment/d3vonn-backend -n d3vonn

# Worker logs
kubectl logs -f deployment/d3vonn-worker -n d3vonn
```
...
2. Check backend logs: `kubectl logs -f deployment/d3vonn-backend -n d3vonn`.
...
2. Check the External Secrets Operator has synced the latest credentials: `kubectl get externalsecret -n d3vonn`.
3. Restart the backend pods: `kubectl rollout restart deployment/d3vonn-backend -n d3vonn`.

### Redis Failure

1. Check Redis pod status: `kubectl get pods -n d3vonn -l app=redis`.
2. If Redis is down, the task queue and rate limiting will be degraded but the API will continue to serve requests.
3. Restart Redis: `kubectl rollout restart deployment/redis -n d3vonn`.
...
```bash
kubectl scale deployment d3vonn-backend --replicas=5 -n d3vonn
```
