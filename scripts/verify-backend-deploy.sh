...
PUBLIC_STATUS=$(curl -s -o /tmp/devonn_public_admin_response.txt -w "%{http_code}" "$BACKEND_URL/api/admin/overview" || true)
echo "HTTP $PUBLIC_STATUS"
cat /tmp/devonn_public_admin_response.txt || true
echo
...
if [[ -n "$NON_ADMIN_JWT" ]]; then
  ...
  NON_ADMIN_STATUS=$(curl -s -o /tmp/devonn_non_admin_response.txt -w "%{http_code}" \
    -H "Authorization: Bearer $NON_ADMIN_JWT" \
    "$BACKEND_URL/api/admin/overview" || true)
  echo "HTTP $NON_ADMIN_STATUS"
  cat /tmp/devonn_non_admin_response.txt || true
  echo
...
if [[ -n "$ADMIN_JWT" ]]; then
  ...
  ADMIN_STATUS=$(curl -s -o /tmp/devonn_admin_response.txt -w "%{http_code}" \
    -H "Authorization: Bearer $ADMIN_JWT" \
    "$BACKEND_URL/api/admin/overview" || true)
  echo "HTTP $ADMIN_STATUS"
  cat /tmp/devonn_admin_response.txt || true
  echo
