...
create table if not exists public.agent_os_approvals (
  ...
  constraint agent_os_approval_agent_scope_nonempty check (
    agent_name is null or length(trim(agent_name)) > 0
  ),
...
create index if not exists agent_os_approvals_workspace_agent_active_idx
  on public.agent_os_approvals (workspace_id, agent_name, action, expires_at desc)
  where revoked_at is null;
