create table if not exists public.jetson_devices (
  ...
  device_class text not null default 'jetson' check (device_class in ('jetson','smart_glasses','companion','robotics')),
  state text not null default 'enrolled' check (state in ('enrolled','online','offline','revoked','quarantined','privacy_locked')),
  hardware_model text,
  firmware_version text,
  model_version text,
  companion_device_id text,
...
create table if not exists public.jetson_telemetry (
  ...
  state text not null check (state in ('enrolled','online','offline','revoked','quarantined','privacy_locked')),
  ...
  gpu_percent double precision check (gpu_percent between 0 and 100),
  memory_percent double precision check (memory_percent between 0 and 100),
...
create index if not exists jetson_devices_class_state_idx on public.jetson_devices(device_class, state);
