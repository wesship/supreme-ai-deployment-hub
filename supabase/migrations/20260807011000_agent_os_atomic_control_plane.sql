...
begin
  ...
  if exists (
    ...
    where length(trim(agent_name)) = 0
  ) then
    raise exception 'disabled agent names must be nonblank';
  end if;

  select coalesce(array_agg(distinct trim(agent_name) order by trim(agent_name)), '{}'::text[])
    into v_disabled_agents
  from unnest(coalesce(p_disabled_agents, '{}'::text[])) as agent_name;

  select * into v_before
  from public.agent_os_workspace_policies
  ...
  insert into public.primetime_audit_events (
    ...
    p_workspace_id,
    p_actor_user_id,
    'agent_os.workspace_policy.updated',
    'agent_os_workspace_policy',
    p_workspace_id,
    jsonb_build_object(
      'before', case when v_before.workspace_id is null then null else to_jsonb(v_before) end,
...
as $$
declare
  v_approval public.agent_os_approvals;
begin
  if p_actor_user_id is null then
    raise exception 'actor_user_id is required';
  end if;
  if p_action is null or length(trim(p_action)) = 0 then
    raise exception 'action must be nonblank';
  end if;
  if p_agent_name is not null and length(trim(p_agent_name)) = 0 then
    raise exception 'agent_name must be null or nonblank';
  end if;
  if p_expires_at is null or p_expires_at <= now() then
    raise exception 'expires_at must be in the future';
  end if;

  insert into public.agent_os_approvals (
    workspace_id,
    action,
    agent_name,
    approved_by,
    ...
    p_workspace_id,
    trim(p_action),
    case when p_agent_name is null then null else trim(p_agent_name) end,
    p_actor_user_id,
    now(),
    p_expires_at,
    p_reason,
  ...
  insert into public.primetime_audit_events (
    ...
    jsonb_build_object(
      ...
      'agent_name', v_approval.agent_name,
      'expires_at', v_approval.expires_at,
      'reason', v_approval.reason,
      'metadata', v_approval.metadata
    )
  );
...
begin
  ...
  update public.agent_os_approvals
  set revoked_at = now()
  ...
  if v_approval.id is null then
    raise exception 'active approval not found for workspace';
  end if;
  ...
  insert into public.primetime_audit_events (
    ...
    jsonb_build_object(
      ...
      'expires_at', v_approval.expires_at,
      'revoked_at', v_approval.revoked_at,
...
comment on function public.agent_os_set_workspace_policy(uuid, boolean, text[], uuid, text) is
  'Atomically updates Agent OS workspace policy and writes PRIMETIME audit evidence.';
comment on function public.agent_os_grant_approval(uuid, text, text, uuid, timestamptz, text, jsonb) is
  'Atomically creates time-bounded Agent OS approval evidence and writes PRIMETIME audit evidence.';
comment on function public.agent_os_revoke_approval(uuid, uuid, uuid, text) is
  'Atomically revokes an Agent OS approval and writes PRIMETIME audit evidence.';
