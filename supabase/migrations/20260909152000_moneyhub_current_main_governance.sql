-- Current-main MoneyHub governance boundary.
-- Preserve the hardened decision that agent_earnings is NOT published to realtime.
-- Browser sessions may read owned rows, while all mutations remain narrowly scoped.
...
-- Realtime is limited to non-financial lifecycle state. agent_earnings remains
-- intentionally excluded; the UI refreshes earnings through authenticated reads.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
