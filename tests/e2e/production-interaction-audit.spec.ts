...
test.describe('D3VONN.IO production interaction audit', () => {
  test('AI Film cards use title-linked preview media and preserve an honest upcoming-title state', async ({ page }) => {
    ...
    const closeFilmDialog = sovereignSignalDialog.locator('button').first();
    await expect(closeFilmDialog).toBeVisible();
    await closeFilmDialog.click();
  ...
  for (const route of PUBLIC_ROUTES) {
    test(`${route} loads and exposes valid interactive controls`, async ({ page, request }) => {
      ...
      for (let index = 0; index < buttonCount; index += 1) {
        ...
        await expect.poll(async () => (await button.boundingBox())?.width ?? 0, { message: `Button "${label}" is too narrow to click on ${route}`, timeout: 5_000 }).toBeGreaterThanOrEqual(20);
        await expect.poll(async () => (await button.boundingBox())?.height ?? 0, { message: `Button "${label}" is too short to click on ${route}`, timeout: 5_000 }).toBeGreaterThanOrEqual(20);
      }
    });
  }
