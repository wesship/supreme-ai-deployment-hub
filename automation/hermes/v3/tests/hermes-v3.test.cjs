...
test("read-only agent cannot merge PR", () => assertEqual(evaluateAgentAction({ agentId: "d3vonn-security-bot" }, "merge_pr").permitted, false));
test("contributor agent can open PR", () => assertEqual(evaluateAgentAction({ agentId: "d3vonn-copilot" }, "open_pr").permitted, true));
test("deployer agent can deploy to staging", () => assertEqual(evaluateAgentAction({ agentId: "d3vonn-deployer" }, "deploy_staging").permitted, true));
test("deployer agent cannot deploy to production", () => assertEqual(evaluateAgentAction({ agentId: "d3vonn-deployer" }, "deploy_production").permitted, false));
...
test("agent firewall blocks IAM modification by contributor", () => {
  const result = evaluateContextFirewall({
    actor: "d3vonn-copilot",
    branch: "feature/test",
    riskSignals: { touchesIAM: true },
    filesChanged: ["infra/iam/main.tf"],
    agentContext: { isAgentActor: true, agentId: "d3vonn-copilot" },
  });
