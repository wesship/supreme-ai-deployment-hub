...
export function routeCommand(parsed: ParsedCommand) {
  ...
  const ranked = routedAgents.map(agent => ({ agent, score: agent.categories.filter(category => categories.includes(category)).length })).filter(item => item.score > 0).sort((a, b) => b.score - a.score);
