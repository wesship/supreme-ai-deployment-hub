...
from backend.agents.governance_context import (
...
def test_platform_admin_does_not_gain_workspace_agent_permissions():
    assert _ROLE_PERMISSIONS["platform_admin"] == set()
...
def test_router_exposes_canonical_paths_and_legacy_aliases():
    ...
    assert "/agents/dispatch" in paths
    assert "/agents/capability" in paths
    assert "/agents/governance/dry-run" not in paths
