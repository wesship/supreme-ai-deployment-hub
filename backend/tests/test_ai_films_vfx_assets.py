...
def test_makebigfilms_provider_requires_governed_project_runtime(monkeypatch):
    ...
    assert entry["required_env"] == ["MAKEBIGFILMS_PROJECT_URL"]
    assert health["capabilities"]["vfx_asset"] is False

    project_url = "https://www.makebigfilms.com/projects/d3vonn-ai-films"
    configured = provider_health({"MAKEBIGFILMS_PROJECT_URL": project_url})
    configured_entry = next(
        item
        for item in configured["providers"]
        if item["provider"] == "makebigfilms" and item["capability"] == "vfx_asset"
    )
    assert configured_entry["status"] == "configured"
    assert configured["capabilities"]["vfx_asset"] is True

    monkeypatch.setenv("MAKEBIGFILMS_PROJECT_URL", project_url)
