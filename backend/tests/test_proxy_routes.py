...
def _test_settings() -> Settings:
    return Settings(
        ...
        require_auth=False,  # Disable auth in tests
        app_env="test",
    )


async def _mock_user_id() -> str:
    """Bypass Supabase JWT validation in tests."""
    return "test-user-id"
...
def override_settings():
    """
    Override get_settings and auth for all tests.
    We must also clear the lru_cache so routers pick up test settings,
    and patch the get_settings reference in each router module directly.
    """
...
class TestChatProxy:
    def test_chat_non_streaming_success(self, client):
        """Non-streaming chat returns OpenAI JSON response."""
        mock_response = {
        ...
        data = resp.json()
        assert data["choices"][0]["message"]["content"] == "Hello from Devonn!"

    def test_chat_invalid_model_rejected(self, client):
        """Request with an unsupported model returns 422."""
        resp = client.post(
            "/api/chat",
            json={
...
class TestRAGProxy:
    ...
    def test_rag_ingest_success(self, client):
        ...
        assert data["chunksIngested"] == 1
        assert data["filename"] == "test.txt"

    def test_rag_ingest_empty_chunks_rejected(self, client):
        """Empty chunks array returns 422."""
        resp = client.post("/api/rag/ingest", json={"filename": "test.txt", "chunks": []})
        assert resp.status_code == 422
...
class TestToolsProxy:
    ...
    def test_github_trigger_success(self, client):
        ...
        data = resp.json()
        assert data["success"] is True
        assert "deploy.yml" in data["message"]

    def test_github_runs_status_success(self, client):
        """GitHub runs status returns workflow run list."""
        mock_runs_data = {
            "total_count": 1,
            "workflow_runs": [
                {
        ...
        data = resp.json()
        assert data["total"] == 1
        assert data["runs"][0]["name"] == "Deploy"
    ...
    def test_n8n_execute_workflow_not_found(self, client):
        ...
        with patch("backend.app.routers.tools.httpx.AsyncClient") as mock_client_cls:
            ...
            mock_resp = MagicMock()
            mock_resp.status_code = 200
            mock_resp.json.return_value = {"data": []}  # empty — no matching workflow
            mock_client.get = AsyncMock(return_value=mock_resp)
    ...
    def test_n8n_execute_success(self, client):
        ...
        with patch("backend.app.routers.tools.httpx.AsyncClient") as mock_client_cls:
            ...
            list_resp.json.return_value = {"data": [{"id": "wf-123", "name": "DeployWorkflow"}]}

            # Second call: execute workflow
            exec_resp = MagicMock()
            exec_resp.status_code = 200
            exec_resp.json.return_value = {"executionId": "exec-456", "status": "running"}
            ...
            resp = client.post(
                "/api/tools/n8n/execute",
                json={"workflow_name": "DeployWorkflow", "payload": {"env": "production"}},
            )

        assert resp.status_code == 200
        data = resp.json()
        assert data["success"] is True
        assert data["workflow"] == "DeployWorkflow"


# ── Health check ──────────────────────────────────────────────────────────────

class TestHealth:
    def test_health_check_not_on_proxy_router(self, client):
        """The /health route is on the main app, not the proxy router."""
        # The proxy router doesn't have /health — it's on the main app
        # This test confirms the proxy router doesn't accidentally shadow it
        resp = client.get("/api/chat")
        # GET on /api/chat should return 405 (method not allowed), not 404
