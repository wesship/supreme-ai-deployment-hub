...
MIGRATION = ROOT / "supabase" / "migrations" / "20260909152000_moneyhub_current_main_governance.sql"
PAGE = ROOT / "src" / "pages" / "MoneyHub.tsx"


def test_financial_rows_are_not_browser_mutable():
...
def test_moneyhub_mutations_use_authenticated_security_definer_rpcs():
    sql = MIGRATION.read_text()
...
def test_moneyhub_keeps_agent_earnings_out_of_realtime():
    sql = MIGRATION.read_text()
    assert "ALTER PUBLICATION supabase_realtime ADD TABLE public.money_agents" in sql
    assert "ALTER PUBLICATION supabase_realtime ADD TABLE public.agent_earnings" not in sql
...
def test_moneyhub_page_does_not_directly_write_financial_tables():
    ...
    assert "table: 'agent_earnings'" not in source
