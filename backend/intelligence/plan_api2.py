...
ALLOWED_REVIEWER_ROLES = {"manager", "compliance", "licensed_representative", "admin"}
...
async def create_review(body: ReviewRequest, user_id: str = Depends(get_current_user_id)):
    parsed = parse_command(body.command)
    if body.reviewer_role not in ALLOWED_REVIEWER_ROLES:
...
async def get_reviews(body: PlanRequest, user_id: str = Depends(get_current_user_id)):
    return {
        "idempotencyKey": body.idempotency_key,
        "records": list_records(body.command),
        "requestedBy": user_id,
