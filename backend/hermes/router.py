...
async def get_tasks(
    ...
    """List Hermes tasks with optional filters."""
...
async def get_single_task(
    ...
    """Get a single Hermes task by ID."""
...
async def dispatch(
    ...
    """Dispatch a task to a named agent via Supabase enqueue-task."""
    task = await get_task(task_id)
    if not task:
        raise HTTPException(404, f"Task {task_id} not found")
    try:
