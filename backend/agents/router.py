...
async def _list_agents_impl() -> list[AgentInfo]:
    return [AgentInfo(name=name, base_url=client.reg.base_url, capabilities=client.reg.capabilities, status=client.status.value)
...
async def governance_dry_run(request: GovernanceDryRunApiRequest, user_id: str = Depends(get_current_user_id)):
    context = await resolve_agent_governance_context(workspace_id=request.workspace_id, user_id=user_id, agent_name=request.agent_name)
    try:
        result: AgentDryRunResult = evaluate_agent_capability_dry_run(AgentDryRunRequest(
            workspace_id=context.workspace_id, actor_id=context.actor_id, agent_name=request.agent_name,
            capability=request.capability, workspace_permissions=context.permissions,
            approved_actions=context.approved_actions, disabled_agents=context.disabled_agents,
            kill_switch_enabled=context.kill_switch_enabled))
    except (KeyError, PermissionError) as exc:
        return GovernanceDryRunApiResponse(workspace_id=context.workspace_id, actor_id=context.actor_id,
            role=context.role, capability=request.capability, agent_name=request.agent_name,
            decision="deny", reason=str(exc), executed=False)
    return GovernanceDryRunApiResponse(workspace_id=context.workspace_id, actor_id=context.actor_id,
        role=context.role, capability=result.capability, agent_name=result.agent_name,
        decision=result.governance.decision.value, reason=result.governance.reason,
        missing_permissions=result.governance.missing_permissions, executed=False)
...
async def dispatch_task(request: DispatchRequest):
    ...
    task = AgentTask(agent_name=request.agent_name, action=request.action, payload=request.payload,
...
async def dispatch_by_capability(request: CapabilityDispatchRequest):
    result = await default_mesh.dispatch_to_capable(capability=request.capability, action=request.action, payload=request.payload)
    if not result.success:
