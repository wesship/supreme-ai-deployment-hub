...
async def update_workspace_policy(request: WorkspacePolicyMutationRequest, user_id: str = Depends(get_current_user_id)):
    membership = await _require_role(request.workspace_id, user_id, _POLICY_ROLES)
    workspace_id = str(membership["workspace_id"])
    result = await set_workspace_policy(workspace_id=workspace_id, kill_switch_enabled=request.kill_switch_enabled,
        disabled_agents=set(request.disabled_agents), actor_user_id=user_id, reason=request.reason)
...
async def create_approval(request: ApprovalGrantRequest, user_id: str = Depends(get_current_user_id)):
    ...
    result = await grant_approval(workspace_id=workspace_id, action=request.action, agent_name=request.agent_name,
        actor_user_id=user_id, expires_at=expires_at, reason=request.reason, metadata=request.metadata)
...
async def revoke_existing_approval(request: ApprovalRevokeRequest, user_id: str = Depends(get_current_user_id)):
    membership = await _require_role(request.workspace_id, user_id, _APPROVAL_ROLES)
    workspace_id = str(membership["workspace_id"])
    result = await revoke_approval(workspace_id=workspace_id, approval_id=request.approval_id, actor_user_id=user_id, reason=request.reason)
