...
async def enrich_with_virustotal(request: PassiveEnrichmentRequest):
    """Read-only reputation enrichment. This endpoint never initiates a scan."""
    try:
        result = await virustotal_enrich(request.indicator_type, request.indicator)
    except PassiveIntelError as exc:
        code = str(exc)
        if code == "virustotal_not_configured":
    ...
    return {"result": result.as_dict(), "activity_class": "passive", "active_scan_performed": False}


@router.get("/graph/projection")
async def get_graph_projection():
    projection = graph_projection()
    return {**projection, "persistence_performed": False, "target_graph": "D3VONN Security Knowledge Graph"}


@router.get("/stix/projection")
async def get_stix_projection():
    return {"spec_version": "2.1", "custom_object_type": "x-d3vonn-security-tool", "objects": stix_projection(), "persistence_performed": False}
