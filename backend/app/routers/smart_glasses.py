...
class _EnvDeviceRegistry:
    async def is_active(self, device_id: str) -> bool:
        allowed = {
            value.strip()
            for value in os.getenv("SMART_GLASSES_DEVICE_IDS", "").split(",")
            if value.strip()
        }
...
class _EnvKillSwitch:
    async def enabled(self) -> bool:
        return os.getenv("SMART_GLASSES_KILL_SWITCH", "false").strip().lower() in {
            "1", "true", "yes", "on"
        }
...
async def _nonce_store() -> RedisNonceStore:
    ...
    if not redis_url:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Smart-glasses replay protection is not configured",
        )
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Smart-glasses replay protection is unavailable",
        ) from exc
...
def _upstream_for(action: str | None) -> tuple[str, str, str | None]:
    if action in {"describe_scene", "read_text"}:
        return (
            os.getenv("SMART_GLASSES_VISION_URL", "").strip(),
            "vision",
            os.getenv("SMART_GLASSES_VISION_TOKEN"),
        )
    return (
        os.getenv("SMART_GLASSES_HERMES_URL", "").strip(),
        "HERMES",
        os.getenv("SMART_GLASSES_HERMES_TOKEN"),
    )
...
async def _forward_remote(envelope: SmartGlassesEnvelope, action: str | None, started: float) -> dict[str, Any]:
    ...
    if not url:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"Smart-glasses {agent} upstream is not configured",
        )
    ...
    try:
        ...
        logger.warning(
            "smart_glasses_upstream_error device_id=%s correlation_id=%s action=%s agent=%s error_type=%s",
            envelope.device_id,
...
@router.post("/execute")
async def execute_smart_glasses_proposal(
    envelope: SmartGlassesEnvelope,
    x_d3vonn_device_key: str | None = Header(default=None, alias="X-D3VONN-Device-Key"),
) -> dict[str, Any]:
    """Authenticate, replay-check, authorize and route one Needle proposal.

    Raw audio/image payloads are intentionally excluded from logs.
    """
    ...
    decision = await authorize_request(
        ...
        device_registry=_EnvDeviceRegistry(),
        kill_switch=_EnvKillSwitch(),
    )
    logger.info(
        "smart_glasses_decision device_id=%s correlation_id=%s action=%s decision=%s reason=%s",
        envelope.device_id,
        envelope.correlation_id,
        decision.action,
        decision.route,
...
async def smart_glasses_health() -> dict[str, Any]:
    """Expose configuration readiness without exposing any secret values."""
    return {
        "status": "ok",
