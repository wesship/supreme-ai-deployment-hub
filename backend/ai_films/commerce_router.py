...
def _check_pollo_rate_limit(user_id: str) -> None:
    ...
    now = time.monotonic(); window = _rate_windows[user_id]
    while window and now - window[0] >= 60: window.popleft()
    if len(window) >= limit: raise HTTPException(status_code=429, detail="Pollo dispatch rate limit exceeded")
    window.append(now)
...
def _signature_candidates(header: str) -> list[str]:
    ...
    for value in header.split():
        parts = value.split(",", 1)
        if len(parts) == 2 and parts[0].strip() == "v1": candidates.append(parts[1].strip())
        elif value.strip(): candidates.append(value.strip())
    return candidates

async def _user_update(access_token: str, table: str, payload: dict[str, Any], row_id: str) -> None:
    base_url = os.getenv("SUPABASE_URL", "").rstrip("/"); anon_key = os.getenv("SUPABASE_ANON_KEY", "").strip()
    if not base_url or not anon_key: raise HTTPException(status_code=503, detail="Commerce persistence is not configured")
    ...
    except httpx.HTTPError as exc: raise HTTPException(status_code=503, detail="Commerce persistence service unavailable") from exc
    if response.status_code >= 400: raise HTTPException(status_code=502, detail="Commerce reservation update failed")

async def _service_request(method: str, path: str, *, json_body: dict[str, Any] | None = None, params: dict[str, str] | None = None) -> Any:
    base_url = os.getenv("SUPABASE_URL", "").rstrip("/"); service_key = os.getenv("SUPABASE_SERVICE_ROLE_KEY", "").strip()
    if not base_url or not service_key: raise HTTPException(status_code=503, detail="Commerce callback persistence is not configured")
    ...
    if response.status_code >= 400: raise HTTPException(status_code=502, detail=f"Commerce persistence failed ({response.status_code})")
    ...
    for platform in request.platforms:
        ...
        for ad_format in request.formats:
            ...
            for variant_number in range(1, request.variants_per_platform + 1):
                ...
                required = "; ".join(request.brand.required_phrases) or "none"; prohibited = "; ".join(request.brand.prohibited_phrases) or "none"; colors = ", ".join(request.brand.colors) or "use brand-appropriate styling"
                prompt = (f"Create a {blueprint['label']} for {request.product.name}. Product description: {request.product.description} "
                ...
                variants.append({"id": f"{platform}-{ad_format}-v{variant_number}", "platform": platform, "format": ad_format, "variant": variant_number,
                    "aspect_ratio": spec["aspect_ratio"], "duration_seconds": spec["default_seconds"], "selling_point": selling_point, "beats": blueprint["beats"], "prompt": prompt,
                    "provider_route": {"provider": "pollo", "model": "pollo-v2-5", "fallback": "openai"}, "jockey_index_after_render": request.index_with_jockey})
...
def list_commerce_templates() -> dict[str, object]: return {"formats": FORMAT_BLUEPRINTS, "platforms": PLATFORM_SPECS}
...
async def plan_campaign(request: CampaignPlanRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
    await _require_user(authorization); return build_campaign_plan(request)
...
async def dispatch_pollo(request: PolloDispatchRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
    ...
    user_id = await _require_user(authorization); _require_pollo_entitlement(user_id); _check_pollo_rate_limit(user_id)
    api_key = os.getenv("POLLO_API_KEY", "").strip()
    if not api_key: raise HTTPException(status_code=503, detail="Pollo is not configured; set POLLO_API_KEY server-side")
    db = SupabaseRLSClient(token)
    reservation = await db.insert("ai_film_commerce_jobs", {"owner_id": user_id, "status": "reserved", "provider": "pollo", "model": "pollo-v2-5", "request": request.model_dump(mode="json"), "handoff_status": "pending"})
    job_id = reservation["id"]
    payload: dict[str, object] = {"input": {"prompt": request.prompt, "length": request.length, "resolution": request.resolution, "mode": request.mode, "generateAudio": request.generate_audio}, "clientSource": "d3vonn-ai-films-commerce"}
    if request.image: payload["input"]["image"] = str(request.image)  # type: ignore[index]
    webhook_url = request.webhook_url or os.getenv("POLLO_WEBHOOK_URL")
    if webhook_url: payload["webhookUrl"] = str(webhook_url)
    url = os.getenv("POLLO_API_BASE_URL", "https://pollo.ai/api/platform").rstrip("/") + "/generation/pollo/pollo-v2-5"
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(url, json=payload, headers={"x-api-key": api_key}); response.raise_for_status()
        result = response.json(); task_id = str(result.get("taskId") or "").strip()
        if not task_id: raise ValueError("Pollo did not return a taskId")
...
async def pollo_webhook(request: Request) -> dict[str, str]:
    ...
    if not secret: raise HTTPException(status_code=503, detail="Pollo webhook verification is not configured")
    webhook_id = request.headers.get("X-Webhook-Id", ""); timestamp = request.headers.get("X-Webhook-Timestamp", ""); signature = request.headers.get("X-Webhook-Signature", ""); body = await request.body()
    try: key = base64.b64decode(secret, validate=True)
    except (ValueError, binascii.Error) as exc: raise HTTPException(status_code=500, detail="Invalid webhook secret encoding") from exc
    signed = webhook_id.encode() + b"." + timestamp.encode() + b"." + body; expected = base64.b64encode(hmac.new(key, signed, hashlib.sha256).digest()).decode()
    if not signature or not any(hmac.compare_digest(candidate, expected) for candidate in _signature_candidates(signature)): raise HTTPException(status_code=401, detail="Invalid Pollo webhook signature")
    try: event = json.loads(body)
    except json.JSONDecodeError as exc: raise HTTPException(status_code=400, detail="Invalid Pollo webhook JSON") from exc
    task_id = str(event.get("taskId") or event.get("task_id") or "").strip()
    if not task_id: raise HTTPException(status_code=400, detail="Pollo webhook task id is required")
    rows = await _service_request("GET", "ai_film_commerce_jobs", params={"task_id": f"eq.{task_id}", "select": "*"})
    if not rows: raise HTTPException(status_code=404, detail="Unknown Pollo task")
    status_value = str(event.get("status") or "processing").lower(); completed = status_value in {"succeeded", "success", "completed", "failed", "cancelled"}
    handoff_status = "queued" if status_value in {"succeeded", "success", "completed"} else "not_applicable"
    output = event.get("output") or (event.get("data") if isinstance(event.get("data"), dict) else {}) or event.get("result") or {}
    await _service_request("PATCH", "ai_film_commerce_jobs", json_body={"status": status_value, "output": output if isinstance(output, dict) else {"value": output}, "provider_event": event,
        "completed_at": datetime.now(timezone.utc).isoformat() if completed else None, "handoff_status": handoff_status,
        "handoff_payload": {"task_id": task_id, "output": output, "targets": ["jockey", "twelvelabs"]} if handoff_status == "queued" else {}}, params={"task_id": f"eq.{task_id}"})
