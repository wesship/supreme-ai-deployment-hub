...
class LocalVideoWorker:
    ...
    The runner receives a stable environment contract. A deployment can point
    WAN or LTX at its own ComfyUI/API/CLI wrapper without changing D3VONN's
    film orchestration layer.
    """
    ...
    def run(self, job: LocalVideoJob, *, timeout_seconds: int = 3600) -> Path:
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError(f"local video generation failed ({completed.returncode}): {stderr}")
