...
async def _persist_event(event: WearableEvent, user_id: str) -> bool:
    ...
    row = {
        "event_id": event.event_id, "event_type": event.event_type, "occurred_at": event.occurred_at.isoformat(),
        "user_id": user_id, "device_id": event.source.device_id, "adapter": event.source.adapter,
        "session_id": event.source.session_id, "correlation_id": event.correlation_id,
        "privacy_classification": event.privacy.classification, "consent": event.privacy.consent,
        "payload": body["payload"], "capabilities": event.capabilities,
        "policy_version": event.audit.policy_version, "trace_id": event.audit.trace_id, "payload_hash": payload_hash,
    }
