...
async def _persist_event(event: WearableEvent, user_id: str) -> bool:
    ...
    row = {
        ...
        "consent": event.privacy.consent,
        "payload": body["payload"],
        "capabilities": event.capabilities,
        "policy_version": event.audit.policy_version,
        "trace_id": event.audit.trace_id,
...
async def ingest_wearable_event(
    event: WearableEvent,
    ...
    return {
        ...
        "event_id": event.event_id,
        "correlation_id": event.correlation_id,
        "next": "queued" if inserted else "already_processed",
    }
