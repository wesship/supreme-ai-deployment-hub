...
@app.middleware("http")
async def preserve_public_cors_on_errors(request: Request, call_next):
    """Keep the allow-list CORS headers on validation/error responses.

    CORSMiddleware handles normal responses and preflight requests. This outer
    guard makes sure a 4xx/5xx generated by validation or an exception handler
    still carries the same explicit production-origin policy, so browser
    telemetry and diagnostics don't become opaque CORS failures.
    """
