...
The Marketplace catalog is backed by `agent_templates`, while user installations are backed by `deployed_agents`. Installation is a governed lifecycle rather than a client-side status update.

## Security boundary

- Public users may read published catalog entries.
- Authenticated users may create installation records for published templates.
...
- MCP tool access is requested capability metadata, not a browser authorization grant.
