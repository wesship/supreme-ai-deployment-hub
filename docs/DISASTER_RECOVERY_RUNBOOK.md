...
```bash
kubectl get pods -n devonn -w
kubectl rollout status deployment/devonn-backend -n devonn
```

**If crash-looping (more than 5 restarts):**
```bash
# Check logs from the crashed container
kubectl logs --previous deployment/devonn-backend -n devonn

# Check for OOMKill (memory limit too low)
kubectl describe pod <pod-name> -n devonn | grep -A5 "Last State"

# Force a clean restart
kubectl rollout restart deployment/devonn-backend -n devonn
```
...
```bash
kubectl get nodes
kubectl get pods -n devonn -o wide   # Check pods redistributed to healthy nodes
```
...
aws autoscaling set-desired-capacity \
  --auto-scaling-group-name devonn-node-group \
...
5. Restart the backend deployment: `kubectl rollout restart deployment/devonn-backend -n devonn`.
