...
5. Check Vapi/ElevenLabs usage, microphone errors, mobile and desktop browser behavior, and the exact commit's CI. The intro is reviewed, while later generated speech is governed by instructions rather than a deterministic script and must be treated as unreviewed output. The model is constrained to voice rehearsal; **it is not connected to the role's approved source content or Hermes role actions**. Do not present this preview as a grounded educator, customer support service, station broadcast, or therapeutic service.
