...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    ...
    description: 'Get the current deployment status of the D3VONN platform, including Vercel frontend, production API service, and Supabase database.',
    parameters: {
      type: 'object',
      properties: {
        service: {
  ...
  },
  {
    ...
    description: 'Execute a D3VONN automation workflow by name (n8n or internal).',
    parameters: {
      type: 'object',
      properties: {
        workflow_name: {
  ...
  {
    name: 'spawn_agent',
    description: 'Spawn a new AI agent with a specific role and task in the D3VONN agent mesh.',
    parameters: {
      type: 'object',
      properties: {
        agent_type: {
  ...
  },
  {
    ...
    description: 'Get system health metrics for the D3VONN platform (token usage, API latency, error rates).',
    parameters: {
      type: 'object',
      properties: {
        metric_type: {
  ...
  },
  {
    ...
    description: 'Search the D3VONN platform documentation and knowledge base.',
    parameters: {
      type: 'object',
      properties: {
        query: {
...
function searchDocumentation(query: string, section = 'all'): unknown {
  return {
    ...
    results: [
      {
        title: 'D3VONN Architecture Overview',
      ...
      },
      {
        ...
        excerpt: 'Agents are orchestrated through the DEVONN API layer. Each agent type (researcher, coder, deployer, monitor) communicates through the platform mesh API.',
