...
describe('backtesting research governance', () => {
  ...
  it('calculates advanced metrics deterministically', () => {
    ...
    const result = runBacktest(bars, DEFAULT_BACKTEST_CONFIG);
    const first = calculateAdvancedMetrics(result);
    const second = calculateAdvancedMetrics(result);
    expect(first).toEqual(second);
    expect(Number.isFinite(first.cagr)).toBe(true);
    expect(Number.isFinite(first.ulcerIndex)).toBe(true);
    expect(first.turnover).toBeGreaterThanOrEqual(0);
  });
});
