...
export function evaluateResearchGate(input: ResearchGateInput): ResearchGateResult {
  ...
  const score = Math.round(100 * (
    Number(sufficientHistory) * 0.2 + Number(sufficientTrades) * 0.2 + Number(drawdownBounded) * 0.1 +
    Number(validationSufficient) * 0.2 + Number(sensitivitySufficient) * 0.15 + Number(costSufficient) * 0.15
  ));
  ...
  return {
    status: input.dataProvenance !== 'governed_historical' ? 'engineering_verification' : (reasons.length ? 'research_inconclusive' : 'research_qualified'),
    score: clamp(score, 0, 100), reasons, checks,
  };
}
...
export function calculateAdvancedMetrics(result: BacktestResult, riskFreeRate = 0.04, barsPerYear = 252) {
  ...
  for (const point of result.equityCurve) {
    ...
    ulcerSquared += drawdownPct ** 2;
  }
  return {
    cagr, calmar, downsideDeviation, payoffRatio, averageWin, averageLoss,
    expectancy: result.trades.length ? result.trades.reduce((s, t) => s + t.pnl, 0) / result.trades.length : 0,
    turnover, ulcerIndex: result.equityCurve.length ? Math.sqrt(ulcerSquared / result.equityCurve.length) : 0,
    excessMeanAnnualized: meanReturn * barsPerYear - riskFreeRate, grossProfit, grossLoss,
  };
}
