...
const MoneyHub = () => {
  ...
  if (loading) {
    return (
      <AppShell>
        <div className="min-h-screen bg-[#060914] px-6 py-24 text-white">
  ...
  return (
    <AppShell>
      <div className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(99,102,241,0.18),transparent_34%),#060914] text-white">
        <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-10">
          <header className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.28em] text-indigo-300">
                <WalletCards className="h-4 w-4" /> D3VONN.IO MoneyHub
              </div>
          ...
          <div className="mb-6 flex items-start gap-3 rounded-xl border border-emerald-400/15 bg-emerald-400/[0.06] px-4 py-3 text-sm text-emerald-100">
            <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />
            <span>Financial records are read-only from the browser. MoneyHub does not enable live brokerage, custody, transfers, or withdrawals.</span>
          </div>
          ...
          <section aria-label="MoneyHub overview" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[
              { label: 'Tracked earnings', value: currency.format(metrics.tracked), icon: CircleDollarSign },
              { label: 'Last 24 hours', value: currency.format(metrics.last24h), icon: TrendingUp },
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-[1.65fr_1fr]">
            <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0b1222]">
              <div className="border-b border-white/[0.07] px-5 py-4">
                <h2 className="font-semibold">Money Agent fleet</h2>
                <p className="mt-1 text-xs text-slate-500">Totals stay server-owned; browser lifecycle controls call governed RPCs.</p>
              </div>
              {agents.length === 0 ? (
                    ...
                    return (
                      <div key={agent.id} className="grid gap-4 px-5 py-4 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center">
                        ...
                        <div className="text-right">
                          <div className="font-medium text-emerald-300">{currency.format(Number(agent.total_earned || 0))}</div>
            ...
            <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0b1222]">
              <div className="border-b border-white/[0.07] px-5 py-4">
                <h2 className="font-semibold">Earnings event ledger</h2>
                <p className="mt-1 text-xs text-slate-500">Operational records only; not a certified accounting ledger.</p>
              </div>
              <div className="divide-y divide-white/[0.06]">
                {earnings.slice(0, 12).map((earning) => (
                  <div key={earning.id} className="px-5 py-4">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="text-sm font-semibold text-emerald-300">{currency.format(Number(earning.amount || 0))}</div>
                    </div>
                  </div>
                ))}
          ...
          <footer className="mt-6 flex flex-col gap-2 text-xs text-slate-600 sm:flex-row sm:items-center sm:justify-between">
            <span>MoneyHub is an operational reporting workspace and does not provide investment advice.</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-labelledby="moneyhub-create-title">
            <form onSubmit={createAgent} className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#0b1222] p-6 shadow-2xl">
              ...
              <div className="space-y-4">
                <label className="block text-sm text-slate-300">Name<input maxLength={120} required value={form.name} onChange={(e) => setForm((value) => ({ ...value, name: e.target.value }))} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 outline-none focus:border-indigo-400/60" /></label>
                <label className="block text-sm text-slate-300">Category<input maxLength={80} value={form.category} onChange={(e) => setForm((value) => ({ ...value, category: e.target.value }))} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 outline-none focus:border-indigo-400/60" /></label>
                <label className="block text-sm text-slate-300">Description<textarea maxLength={1000} rows={4} value={form.description} onChange={(e) => setForm((value) => ({ ...value, description: e.target.value }))} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 outline-none focus:border-indigo-400/60" /></label>
              </div>
