...
export default function CustomLists() {
  ...
  const createList = () => {
    const displayName = window.prompt("List name");
  ...
  const editList = (item: CustomList) => {
    const displayName = window.prompt("List name", item.displayName);
    if (!displayName?.trim()) return;
    const description = window.prompt("Description", item.description) ?? item.description;
    setLists((current) => current.map((entry) => entry.id === item.id ? {
      ...entry,
      displayName: displayName.trim(),
      description: description.trim(),
      updatedAt: new Date().toISOString(),
  ...
  const archiveList = (id: string) => {
    if (!window.confirm("Archive this custom list?")) return;
    setLists((current) => current.map((entry) => entry.id === id ? { ...entry, archived: true } : entry));
    setSelected((current) => {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-slate-100 text-slate-950">
      <header className="sticky top-16 z-30 flex h-14 items-center gap-3 bg-blue-800 px-3 text-white shadow-md lg:px-5">
        <button className="rounded-md p-2 hover:bg-white/10 lg:hidden" onClick={() => setMobileNav(true)} aria-label="Open CRM navigation">
          <Menu className="h-5 w-5" />
        </button>
        <div className="flex min-w-0 items-center gap-3">
          <div className="grid h-8 w-8 place-items-center rounded-lg bg-white font-black text-blue-800">D3</div>
          <div className="hidden sm:block">
            <p className="text-sm font-bold leading-none">DEVONN CRM</p>
            <p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-blue-100">PRIMETIME on d3vonn.io</p>
          </div>
        </div>
        <button className="ml-2 hidden min-w-40 items-center justify-between rounded-md border border-white/20 bg-white/10 px-3 py-2 text-xs font-medium md:flex">
          D3VONN Main Workspace <ChevronRight className="h-3.5 w-3.5" />
        </button>
        <label className="ml-auto hidden w-full max-w-md items-center gap-2 rounded-md bg-white/10 px-3 py-2 md:flex">
          <Search className="h-4 w-4 text-blue-100" />
          <input className="w-full bg-transparent text-sm text-white placeholder:text-blue-100/70 focus:outline-none" placeholder="Search contacts, leads, tasks..." />
        </label>
        ...
        <aside className="hidden w-20 shrink-0 border-r border-slate-200 bg-white lg:block">
          <nav className="sticky top-30 flex flex-col items-center gap-1 py-3" aria-label="CRM navigation">
            {navItems.map(([label, to, Icon]) => (
              <NavLink key={label} to={to} end={to === "/crm"} className={({ isActive }) => `group flex w-16 flex-col items-center rounded-lg px-1 py-2 text-[10px] font-medium transition ${isActive ? "bg-blue-50 text-blue-800" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}>
                <Icon className="mb-1 h-5 w-5" /><span className="max-w-full truncate">{label}</span>
              </NavLink>
            ))}
          </nav>
        </aside>

        {mobileNav && <div className="fixed inset-0 z-50 bg-slate-950/50 lg:hidden" onClick={() => setMobileNav(false)}>
          <aside className="h-full w-72 bg-white p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between"><strong>DEVONN CRM</strong><button onClick={() => setMobileNav(false)} aria-label="Close CRM navigation"><X /></button></div>
            <nav className="space-y-1">{navItems.map(([label, to, Icon]) => <NavLink key={label} to={to} onClick={() => setMobileNav(false)} className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-800"><Icon className="h-4 w-4" />{label}</NavLink>)}</nav>
          </aside>
        </div>}
        ...
        <section className="min-w-0 flex-1 p-3 sm:p-5 lg:p-7">
          <div className="mx-auto max-w-[1500px]">
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button onClick={createList} className="inline-flex items-center justify-center gap-2 rounded-md bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-800"><Plus className="h-4 w-4" />Create New Custom List</button>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[860px] border-collapse text-left text-sm">
                  <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                    <tr>
                      <th className="w-12 px-4 py-3"><input type="checkbox" checked={visibleLists.length > 0 && selected.size === visibleLists.length} onChange={toggleAll} aria-label="Select all lists" /></th>
                      <th className="px-4 py-3"><button onClick={() => setSortAsc((value) => !value)} className="font-semibold hover:text-blue-700">Display Name {sortAsc ? "↑" : "↓"}</button></th>
                      <th className="px-4 py-3 font-semibold">Description</th>
                      <th className="px-4 py-3 text-right font-semibold">Record Count</th>
                      <th className="px-4 py-3 font-semibold">Last Updated</th>
                      <th className="w-28 px-4 py-3 text-right font-semibold">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {visibleLists.map((item) => <tr key={item.id} className="hover:bg-blue-50/40">
                      <td className={`px-4 ${dense ? "py-2.5" : "py-4"}`}><input type="checkbox" checked={selected.has(item.id)} onChange={() => toggleOne(item.id)} aria-label={`Select ${item.displayName}`} /></td>
                      <td className={`px-4 font-semibold text-slate-900 ${dense ? "py-2.5" : "py-4"}`}>{item.displayName}</td>
                      <td className={`max-w-xl px-4 text-slate-600 ${dense ? "py-2.5" : "py-4"}`}>{item.description}</td>
                      <td className={`px-4 text-right tabular-nums ${dense ? "py-2.5" : "py-4"}`}>{item.recordCount.toLocaleString()}</td>
                      <td className={`px-4 whitespace-nowrap text-slate-500 ${dense ? "py-2.5" : "py-4"}`}>{formatDate(item.updatedAt)}</td>
                      <td className={`px-4 ${dense ? "py-2.5" : "py-4"}`}><div className="flex justify-end gap-1"><button onClick={() => editList(item)} className="rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-blue-700" aria-label={`Edit ${item.displayName}`}><MoreHorizontal className="h-4 w-4" /></button><button onClick={() => archiveList(item.id)} className="rounded-md p-2 text-slate-500 hover:bg-red-50 hover:text-red-700" aria-label={`Archive ${item.displayName}`}><Trash2 className="h-4 w-4" /></button></div></td>
                    </tr>)}
