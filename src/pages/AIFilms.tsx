    ...
    <PublicPageShell breadcrumbs={breadcrumbs}>
      <AnimatePresence>
        {showIntro && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            role="dialog"
            aria-label="D3VONN Studios cinematic introduction"
          >
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(20,105,255,0.35),transparent_42%),linear-gradient(135deg,#02040a,#07152f_55%,#000)]" />
            <motion.div
            ...
            >
              ...
              <motion.h1
                className="bg-gradient-to-b from-white via-cyan-100 to-blue-500 bg-clip-text text-5xl font-black tracking-tight text-transparent sm:text-7xl lg:text-8xl"
              ...
              <motion.p
                className="mx-auto mt-6 max-w-2xl text-base text-blue-100/80 sm:text-xl"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <Button onClick={closeIntro} size="lg" className="min-w-40">
                  <Play className="mr-2 h-5 w-5" /> Enter Studios
                </Button>
                <Button onClick={() => setMuted((value) => !value)} variant="outline" size="icon" aria-label={muted ? 'Unmute intro' : 'Mute intro'}>
                  {muted ? <VolumeX className="h-5 w-5" /> : <Volume2 className="h-5 w-5" />}
                </Button>
              </motion.div>
            </motion.div>
            <Button onClick={closeIntro} variant="ghost" className="absolute right-4 top-4 text-white" aria-label="Skip intro">
              <X className="mr-2 h-4 w-4" /> Skip Intro
            </Button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <section aria-label="D3VONN AI Films" className="overflow-hidden bg-background">
        <div className="relative min-h-[72vh] border-b border-border/60">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_30%,rgba(20,105,255,0.35),transparent_30%),radial-gradient(circle_at_30%_20%,rgba(34,211,238,0.12),transparent_24%),linear-gradient(120deg,#030711_10%,#07162f_58%,#020409)]" />
          ...
          <div className="container relative z-10 mx-auto flex min-h-[72vh] items-end px-4 pb-16 pt-28 sm:px-6 lg:pb-24">
            <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Button size="lg" onClick={scrollToStudio}>
                  <Sparkles className="mr-2 h-5 w-5" /> Create a Film
                </Button>
                <Button size="lg" variant="outline" onClick={() => setShowIntro(true)}>
                  <Play className="mr-2 h-5 w-5" /> Replay Intro
                </Button>
        ...
        <div className="container mx-auto px-4 py-12 sm:px-6 lg:py-16">
          <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="relative w-full lg:max-w-sm">
              ...
              <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search movies, topics, or categories" className="pl-10" />
            </div>
          </div>

          <div className="mb-8 flex gap-2 overflow-x-auto pb-2" aria-label="Film categories">
            ...
            {visibleMovies.map((movie, index) => (
              ...
              >
                <div className="relative aspect-video overflow-hidden bg-[radial-gradient(circle_at_70%_25%,rgba(34,211,238,.45),transparent_25%),linear-gradient(135deg,#06142b,#02040a_70%)]">
                  <div className="absolute inset-0 transition duration-500 group-hover:scale-110 group-hover:bg-primary/10" />
                  <Film className="absolute left-6 top-6 h-9 w-9 text-cyan-200/80" />
                  <Button size="icon" className="absolute bottom-5 right-5 rounded-full" aria-label={`Play ${movie.title}`}>
          ...
          {visibleMovies.length === 0 && (
            <div className="rounded-2xl border border-dashed border-border p-12 text-center text-muted-foreground">No films match this search.</div>
          )}
        </div>
