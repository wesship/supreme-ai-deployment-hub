...
export default function PrimetimeCustomLists() {
  ...
  async function loadLists(currentWorkspaceId = workspaceId) {
    ...
    setError('');
    try {
      setLists(await primetimeRelease1Api.listCustomLists(currentWorkspaceId, includeArchived));
    } catch (cause) {
  ...
  async function submitEditor(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editor.list) {
        await primetimeRelease1Api.updateCustomList(editor.list.id, workspaceId, {
          display_name: displayName.trim(),
          description: description.trim(),
        });
      } else {
        await primetimeRelease1Api.createCustomList({
  ...
  async function archiveList() {
    ...
    setError('');
    try {
      await primetimeRelease1Api.archiveCustomList(archiveCandidate.id, workspaceId);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-slate-300">
                Build workspace-scoped contact groups with authenticated access, role enforcement, soft archival, derived member counts, and immutable audit history.
              </p>
        ...
        <section className="rounded-2xl border border-white/10 bg-slate-900/70 shadow-xl shadow-black/20">
          ...
          <div className="grid gap-4 border-b border-white/10 p-4 sm:grid-cols-3">
            ...
            <div className="rounded-xl bg-white/5 p-4"><ShieldCheck className="h-5 w-5 text-amber-300" /><p className="mt-2 text-sm font-semibold">Server governed</p><p className="text-sm text-slate-400">Membership, roles, audit, soft archive</p></div>
          </div>
      ...
      <Dialog open={editor.open} onOpenChange={(open) => setEditor((current) => ({ ...current, open }))}>
        <DialogContent>
          <form onSubmit={submitEditor} className="space-y-5">
            <DialogHeader><DialogTitle>{editor.list ? 'Edit custom list' : 'Create custom list'}</DialogTitle><DialogDescription>Changes are workspace-scoped, role-checked by the API, and recorded in the audit ledger.</DialogDescription></DialogHeader>
