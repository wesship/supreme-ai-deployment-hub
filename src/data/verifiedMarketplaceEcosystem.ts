...
const videoStartupAnalyst: AgentTemplate = {
  ...
  status: 'published', version: '1.0.0', icon: '📈',
  ...
  createdAt: '2026-08-20', updatedAt: '2026-08-20', featured: true,
};
...
canonicalAgents.push(videoStartupAnalyst);

const duplicateIds = canonicalAgents.map((agent) => agent.id).filter((id, index, ids) => ids.indexOf(id) !== index);
if (duplicateIds.length > 0) throw new Error(`Duplicate Marketplace agent IDs: ${[...new Set(duplicateIds)].join(', ')}`);
if (canonicalAgents.length !== 16) throw new Error(`Marketplace ecosystem integrity failure: expected 16 agents, found ${canonicalAgents.length}`);
