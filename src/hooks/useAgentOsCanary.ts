...
export function useAgentOsCanary() {
    ...
    (workspaceId: string, canaryRunId?: string) => {
      ...
      return guarded(() =>
        requestJson<CanaryStatus>(
          `/agents/governance/control/canary/status?workspace_id=${encodeURIComponent(workspaceId)}${suffix}`
        )
      );
    },
    [guarded]
  ...
  const startCanaryLease = useCallback(
    (workspaceId: string, disabledAgents: string[], leaseSeconds: number, reason: string) =>
      guarded(() =>
        requestJson<Record<string, unknown>>('/agents/governance/control/canary/lease', {
          method: 'POST',
          body: JSON.stringify({
            workspace_id: workspaceId,
            disabled_agents: disabledAgents,
