...
const DeployAgentModal: React.FC<DeployAgentModalProps> = ({ agent, open, onClose, onDeployComplete }) => {
  ...
  useEffect(() => {
    if (agent) setConfig({ name: `${agent.slug}-1`, environment: 'development', notifications: {} });
    setEnableNotifications(false); setDeployedId(null); setStep('config');
  }, [agent]);
  ...
  const handleDeploy = async () => {
    ...
    try {
      const deployed = await deployMutation.mutateAsync({
        ...
        mcp_config: {
          enabled_tools: agent.slug === 'openmontage-video-intelligence-studio' ? ['hermes', 'ffmpeg', 'media-provider', 'publish'] : [],
        },
      });
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-w-lg bg-background/95 backdrop-blur-xl border-border/50">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Rocket className="w-5 h-5 text-primary" />Deploy {agent.name}</DialogTitle>
          <DialogDescription>Creates a governed installation record and enables the agent for your workspace.</DialogDescription>
        </DialogHeader>
