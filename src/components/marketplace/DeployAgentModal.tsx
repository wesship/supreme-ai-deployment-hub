    ...
    try {
      const deployed = await deployMutation.mutateAsync({
        ...
        config: {
          ...
          notifications: enableNotifications ? config.notifications : {},
          integrations: agent.integrations ?? [],
          requirements: agent.requirements ?? [],
          openmontage: agent.slug === 'openmontage-video-intelligence-studio',
        },
        mcp_config: {
          enabled_tools: agent.slug === 'openmontage-video-intelligence-studio'
            ? ['hermes', 'ffmpeg', 'media-provider', 'publish']
            : [],
        },
      });
      setDeployedId(deployed?.id ?? null);
      setStep('complete');
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup
                ...
                onValueChange={(environment: AgentDeploymentConfig['environment']) =>
                  setConfig({ ...config, environment })
                }
                className="flex gap-4"
              >
                {(['development', 'staging', 'production'] as const).map((environment) => (
                  <div className="flex items-center space-x-2" key={environment}>
                    <RadioGroupItem value={environment} id={environment} />
                    <Label htmlFor={environment} className="font-normal capitalize cursor-pointer">
                      {environment}
                    </Label>
                  </div>
                ))}
            ...
            {enableNotifications && (
              <Input
                ...
                onChange={(event) =>
                  setConfig({
                    ...config,
                    notifications: { email: event.target.value ? [event.target.value] : undefined },
                  })
                }
              />
            )}
            ...
            <div className="flex items-center justify-between rounded-lg bg-secondary/30 p-4">
              ...
              <div className="text-xl font-bold text-primary">{getPricingLabel(agent.pricing)}</div>
            </div>

            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={onClose}>Cancel</Button>
              <Button onClick={() => setStep('review')} disabled={!config.name.trim()}>
                Review & Deploy
              </Button>
            </div>
          </div>
        ...
        {step === 'review' && (
          <div className="space-y-6 py-4">
            <div className="space-y-3">
              ...
              <div className="flex justify-between rounded-lg border border-primary/20 bg-primary/10 p-3">
                <span className="font-medium">Total</span>
                <span className="font-bold text-primary">{getPricingLabel(agent.pricing)}</span>
              </div>
            </div>
            <div className="flex justify-end gap-2">
          ...
          <div className="space-y-4 py-12 text-center">
            ...
            <div>
              ...
              <div className="mt-1 text-sm text-muted-foreground">
                {agent.name} is registered as {config.name}.
              </div>
            </div>
            <div className="flex justify-center gap-2 pt-4">
              <Button variant="outline" onClick={onClose}>Close</Button>
              <Button onClick={() => navigate(agent.slug === 'openmontage-video-intelligence-studio' ? '/film' : `/agents${deployedId ? `?agent=${deployedId}` : ''}`)}>
                <Server className="mr-2 h-4 w-4" />
                {agent.slug === 'openmontage-video-intelligence-studio' ? 'Open Film Studio' : 'View Agent'}
              </Button>
            </div>
          </div>
        )}
