...
export function PrimetimeCustomListMembersDialog({ workspaceId, list, onOpenChange, onChanged }: Props) {
  ...
  async function load() {
    ...
    try {
      const [memberRows, peopleRows] = await Promise.all([
        primetimeRelease1Api.listCustomListMembers(list.id, workspaceId),
  ...
  async function add(personId: string) {
    ...
    setError('');
    try {
      await primetimeRelease1Api.addCustomListMember(list.id, workspaceId, personId);
      await load();
      onChanged();
    } catch (cause) {
  ...
  async function remove(personId: string) {
    ...
    setError('');
    try {
      await primetimeRelease1Api.removeCustomListMember(list.id, personId, workspaceId);
      await load();
      onChanged();
    } catch (cause) {
    ...
    <Dialog open={Boolean(list)} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-4xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Add or remove people within the active workspace. Changes are role-checked, soft-removed, and written to the audit ledger.
          </DialogDescription>
        </DialogHeader>
